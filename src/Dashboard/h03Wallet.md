...
- #### Polkadot 钱包
  ...
  - 激活账户：激活账户需要充入保活押金 (Existential Deposit)，即创建完成之后由其他账户给你充入 0.01 个 DOT，你的账户才能正常使用
  - 账户回收 (Reaped)：当你的账户余额低于 0.01 个 DOT 之后，账户会被回收，需要再次充入 0.01 个 DOT 才能重新激活 
...
- #### MetaMask 钱包
  ...
  - [MetaMask.io](https://link.zhihu.com/?target=https%3A//metamask.io/) 以太坊网络开源钱包，目前有 Chrom Firefox 等网页插件和 APP 版本。是比较大众的 ETH 插件钱包，基本上网页版本的 Dapp 普遍支持。
...
- #### 新增钱包
  ...
  - 创建钱包名称
    - 钱包名称验证长度，最高不能超过 24 位。超出后，错误显示：**钱包名超出最大长度 24**
...
- 合约交互前，需要将用户的使用的资产对合约进行 Approve。ETH 资产除外。Approve 分为两类：
  ...
  - 过度 Approve ，即用户可以进行合约对该钱包，无限额度授予。用户可以后期重新调整 Appove 的值。chu
...
根据当前钱包的地址，进行资产的换取。目前直接支持 layer1 层的合约协议。
