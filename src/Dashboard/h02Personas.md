...
- 备份时间提醒，每过一个自然月提醒一次，删除后当月不显示提醒。点击后跳转至 Settings 中备份。
...
- #### 上传
  ...
  - 上传文件，每个用户上传文件，每个文件不能超出 10.5MB 。每个 Personas 每天上传不能超出 50。超出后 Upload 按钮置灰。

- #### 创建文件夹

  - 创建文件夹上限 1000（待定）个。超出 Create new folder 按钮置灰。
  - 文件夹名称字数不超过 128 英文字符/64 个中文字符。超出后无法继续输入。
